...
public class GameBoard extends JPanel {
    private Model model;
    private  int startX = 5; //Vasak ülemine x-kordinaat
    private  int startY = 5; //Vaska ülemine y-kordinaat
    ...
    private void drawRowColumn(Graphics g) {
        ...
        for(int y = startY + height; y < (height * model.getBoardSize()) + height; y+= height) {
            g.drawRect(startX, y, width, height); // Joonistab ruudud 30 x 30 pikslit
            if( i  < 10 ) {
    ...
    private void drawColumnAlphabet(Graphics g) {
        ...
        g.setColor(Color.WHITE); //Tähestiklu joonistamisw värv

        for( int x = startX; x <= (width * model.getBoardSize()) + width; x += width) {
            g.drawRect(x, startY, width, height); //Joonista ruut 30x30 11tk
            if(x > startX) {
